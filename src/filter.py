    ...
    def load_config(self):
        ...
        try:
            with open(self.config_path, "r", encoding="utf-8") as f:
                ...
                if config:
                    filters_section = config.get("filters") or config.get("global_filters") or {}
                    self.enabled = filters_section.get("enabled", True)
                    self.exclude_keywords = filters_section.get("exclude_keywords", [])
            logger.info(f"Küresel filtreler yüklendi (Etkin: {self.enabled}). {len(self.exclude_keywords)} adet dışlama kelimesi tanımlı.")
